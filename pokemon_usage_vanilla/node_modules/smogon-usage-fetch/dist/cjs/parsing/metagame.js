...
exports.metagameFromString = void 0;
...
const convertFrequencyPair = (str, paddingRegex = /(\s+)\d/) => {
    const padding = regex_1.getMatchGroup(str, paddingRegex, 0);
    const splitStr = str.split(padding);
    const name = splitStr[0].trim();
    const frequency = frequency_1.convertFrequency(splitStr[1]);
 ...
 * @return parsed page.
 */
const metagameFromString = (page) => {
    const rows = page.split("\n");
    const separatorIndex = rows.findIndex(lightdash_1.isBlank);
    if (separatorIndex === -1) {
    ...
    return {
        ...
        stalliness: {
            mean: Number(regex_1.getMatchGroup(stallinessMeanRow, STALLINESS_MEAN_REGEX, 1)),
            one: frequency_1.convertFrequency(regex_1.getMatchGroup(stallinessOneRow, STALLINESS_ONE_REGEX, 1)),
        },
    };
};
exports.metagameFromString = metagameFromString;
