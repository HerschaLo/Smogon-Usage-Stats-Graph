...
exports.timeframeToString = exports.timeframeFromString = void 0;
/**
 * @private
 */
...
const timeframeFromString = (timeframeLine) => {
    ...
    if (split.length < itemsMin || split.length > itemsMax) {
        throw new Error(`Not a valid timeframe: '${timeframeLine}', expecting between ${itemsMin} and ${itemsMax} sub-elements but got ${split.length}.`);
    }
    const year = split[0];
    const month = split[1];
    const modifier = split[2];
    if (modifier != null) {
        return {
            year,
            month,
 ...
 */
const timeframeToString = (timeframe) => {
